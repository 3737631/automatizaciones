'use client';

import { useState } from 'react';
import { Mail, CheckCircle, ArrowRight, AlertCircle } from 'lucide-react';
import { cn } from '@/lib/utils';

interface EmailConnectCardProps {
  onConnected?: (email: string) => void;
}

export default function EmailConnectCard({ onConnected }: EmailConnectCardProps) {
  const [mode, setMode] = useState<'choice' | 'mock'>('choice');
  const [emailInput, setEmailInput] = useState('');
  const [connectedEmail, setConnectedEmail] = useState<string | null>(null);
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);

  const handleMockConnect = () => {
    if (loading) return;
    setError('');
    const trimmed = emailInput.trim();
    if (!trimmed) {
      setError('Introduce un correo electrónico');
      return;
    }
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(trimmed)) {
      setError('Correo electrónico no válido');
      return;
    }
    setLoading(true);
    setTimeout(() => {
      localStorage.setItem('autocandidatura_connected_email', trimmed);
      setConnectedEmail(trimmed);
      setLoading(false);
      onConnected?.(trimmed);
    }, 800);
  };

  const handleGoogleConnect = () => {
    setMode('mock');
  };

  if (connectedEmail) {
    return (
      <div className="bg-white rounded-xl border border-green-200 shadow-sm p-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center">
            <CheckCircle className="w-5 h-5 text-green-600" />
          </div>
          <div>
            <p className="text-sm font-medium text-green-800">Correo conectado</p>
            <p className="text-sm text-green-600">{connectedEmail}</p>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow p-6">
      <div className="flex flex-col items-center text-center gap-4">
        <div className="w-14 h-14 rounded-full bg-red-50 flex items-center justify-center">
          <Mail className="w-7 h-7 text-red-500" />
        </div>
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Conecta tu correo</h3>
          <p className="text-sm text-gray-500 mt-1">
            Necesitamos acceso a tu Gmail para enviar candidaturas
          </p>
        </div>

        {mode === 'choice' && (
          <button
            onClick={handleGoogleConnect}
            className="flex items-center gap-2 px-6 py-2.5 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" />
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
            </svg>
            Conectar Gmail
          </button>
        )}

        {mode === 'mock' && (
          <div className="w-full max-w-sm space-y-3">
            <input
              type="email"
              value={emailInput}
              onChange={(e) => { setEmailInput(e.target.value); setError(''); }}
              placeholder="tu@email.com"
              className={cn(
                'w-full px-4 py-2.5 border rounded-lg text-sm outline-none transition-colors',
                error ? 'border-red-300 focus:ring-2 focus:ring-red-200' : 'border-gray-300 focus:ring-2 focus:ring-blue-200 focus:border-blue-400'
              )}
              onKeyDown={(e) => e.key === 'Enter' && handleMockConnect()}
            />
            {error && (
              <p className="text-xs text-red-500 flex items-center gap-1">
                <AlertCircle className="w-3 h-3" />
                {error}
              </p>
            )}
            <button
              onClick={handleMockConnect}
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors disabled:opacity-50"
            >
              {loading ? 'Conectando...' : 'Conectar'}
              {!loading && <ArrowRight className="w-4 h-4" />}
            </button>
          </div>
        )}
      </div>
    </div>
  );
}
